...
rx_module.setChannel(110); //set the channel to 110 to avoid interference
rx_module.setDataRate(RF24_250KBPS); //set the data rate to lowest value for higher range
...
//rx_module.setPayloadSize(32);
rx_module.openReadingPipe (1, 0x7878787878LL);
...
delay(100); //stability pause
if (rx_module.available()) {
rx_module.read(&nrf_data_packet, 30);
...
else {Serial.print("No transmission");}
}